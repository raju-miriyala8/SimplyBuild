...
function BuilderPage() {
  ...
  const isDraftProject = !projectId || projectId === 'new';
  ...
  useEffect(() => {
    const loadProject = async () => {
      if (isDraftProject) {
        setProject({ title: 'Untitled Project', messages: [], generatedCode: '' });
        setMessages([]);
        setCode('');
    ...
    loadProject();
  }, [projectId, isDraftProject, navigate, showToast]);
  ...
  const handleSend = async (prompt) => {
    ...
    try {
      ...
      if (isDraftProject) {
        const draftTitle = prompt.length > 50 ? `${prompt.substring(0, 50)}...` : prompt;
        const createdProject = await createProject(draftTitle);
        activeProjectId = createdProject._id;
        activeProject = createdProject;
        setProject(createdProject);
        setEditTitle(createdProject.title || draftTitle || 'Untitled Project');
      }

      const result = await generateCode(activeProjectId, prompt);
      ...
      if ((activeProject?.title || 'Untitled Project') === 'Untitled Project') {
        const newTitle = prompt.length > 50 ? prompt.substring(0, 50) + '...' : prompt;
        setProject((prev) => ({ ...prev, title: newTitle }));
        setEditTitle(newTitle);
      }

      if (isDraftProject && activeProjectId) {
        navigate(`/builder/${activeProjectId}`, { replace: true });
      }
    } catch (err) {
